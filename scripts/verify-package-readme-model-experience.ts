...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-mcp': { kind: 'indirect', reason: 'The bind dock routes mcpManager bind/unbind RPCs; the host mcp-manager and mcp-client bridge own the model-visible tool and schema registrations.' },
  ...
  'packages/bundle/mcp-dsh': { kind: 'indirect', reason: 'The bundle is a patch-list carrier that inserts mcp-manager; the manager and its mcp-client bridges own the model-visible tool registrations.' },
  'packages/bundle/higress-trace': { kind: 'none', reason: 'The optional bundle composes tracing and reverse-query plugins without adding model-visible context.' },
  'packages/gateway/gateway-trace': { kind: 'none', reason: 'The reverse-query service stores log-only observations outside model history.' },
  'packages/gateway/gateway-trace-query': { kind: 'none', reason: 'The service-account provider only retrieves sanitized gateway observations after a request.' },
  'packages/gateway/gateway-trace-console': { kind: 'none', reason: 'The Console provider only retrieves sanitized gateway observations after a request.' },
  'packages/telemetry/telemetry': { kind: 'none', reason: 'The tracing service propagates process-local context without changing model input.' },
  'packages/telemetry/telemetry-otel': { kind: 'none', reason: 'The OTLP provider exports local spans without changing model input.' },
  ...
  'packages/mcp/mcp-manager': { kind: 'indirect', reason: 'The management service mounts mcp-client instances onto each agent scope; the client bridge owns the model-visible tool registrations.' },
